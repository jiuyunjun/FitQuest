...
export function Data() {
  ...
  return (
    <div style={{ padding: '20px 18px 28px', display: 'grid', gap: 20, alignContent: 'start' }}>
      <div style={{ font: `400 11px/1 ${F.pixel}`, color: C.gold }}>本周</div>

      <div style={{ border: `3px solid ${C.border}`, background: C.panel, padding: 18, display: 'grid', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <span style={{ font: `400 9px/1 ${F.pixel}`, color: C.gold }}>FITNESS BANK</span>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 88 }}>
          {week.map((d, i) => {
            ...
            return (
              <div key={d.date} style={{ flex: 1, display: 'grid', gap: 6, justifyItems: 'center' }}>
                <div style={{ width: '100%', height: h, background: color }} />
                <span style={{ font: `400 8px/1 ${F.pixel}`, color: C.label }}>{DAY_LABELS[i]}</span>
              </div>
            )
          })}
        </div>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        <Label>最近训练</Label>
        {state.sessions.length === 0 && (
          <div style={{ font: `400 13px/1.8 ${F.sans}`, color: C.muted }}>
            还没有记录。完成一次训练后会出现在这里。
          </div>
        )}
          ...
          <Row
            ...
            left={
              <div style={{ display: 'grid', gap: 5 }}>
                <div style={{ font: `700 13px/1 ${F.sans}`, color: C.text }}>
                  {EXERCISES[s.exercise].name} · {s.reps} 次
                </div>
                <div style={{ font: `400 10px/1 ${F.mono}`, color: C.muted }}>
                  {s.date.slice(5)} · {s.durationSeconds}s · Q {s.quality || '—'}
                  {s.bossKilled ? ' · 击杀' : ''}
                </div>
              </div>
            }
            right={<div style={{ font: `400 9px/1 ${F.pixel}`, color: C.gold }}>+{s.xp} XP</div>}
          />
        ))}
...
function Tile({ label, value, color = C.text }: { label: string; value: number; color?: string }) {
  return (
    <div style={{ border: `2px solid ${C.line}`, background: C.raised, padding: 14, display: 'grid', gap: 8 }}>
      <span style={{ font: `400 8px/1 ${F.pixel}`, color: C.label }}>{label}</span>
      <span style={{ font: `400 20px/1 ${F.pixel}`, color }}>{value}</span>
    </div>
  )
}
